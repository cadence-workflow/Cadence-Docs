...
We see a spike in both errors and warnings, lets group the logs by the message,
and the error:
...
that:](https://github.com/cadence-workflow/cadence/blob/master/service/matching/tasklist/task_list_manager.go#L293-L304)
...
- On line 7 we add one to the stopWG wait group. A wait group in go enables us
  to wait for a group of goroutines to finish. In this case we are adding the go
...
- In the `notifyPartitionConfig` method we make an RPC call, and due to the DB
  unavailability this call fails, however, since we use the background context
  we just wait for the response forever.
...
blocks heart beating indefinitely!
