...
- Failover changes which of those clusters is **active**. Active and standby status is per domain, not per cluster. **Cadence domains are active-active by default**. Users can specify where a workflow should be processed using cluster attributes. If no cluster attribute is specified, the domain's active cluster is used for processing.
