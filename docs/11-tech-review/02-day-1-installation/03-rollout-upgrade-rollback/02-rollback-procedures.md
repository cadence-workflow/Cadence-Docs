...
| Dynamic configuration | Revert the value | Reloaded without a restart. Revert this before the binary when the regression came from a config change. |
...
3. **Redeploy the previous version.** Use the same image tag, static configuration, and `numHistoryShards` the cluster ran before the upgrade. Replace processes gradually so History shards can move to surviving hosts between batches. The order of Frontend, History, Matching, and Worker does not matter, since mixed versions across roles are compatible.
...
`helm rollback <release> <revision>` restores the chart, values, and image tag of an earlier release revision. The chart's schema Job runs again with the previous image. Because the database is already newer, `update-schema` does nothing, and the [setup script](https://github.com/cadence-workflow/cadence-charts/blob/main/charts/cadence/files/schema/setup-postgres.sh) logs `Rollback is not allowed` and exits successfully. The schema is not downgraded.

Each server pod in the chart also runs a `wait-for-schema` init container before Cadence starts. It reads the schema version bundled in the pod's image and [waits until the database reports that version](https://github.com/cadence-workflow/cadence-charts/blob/main/charts/cadence/files/schema/wait-schema-postgres.sh). Unlike the server's own check, this one requires a match rather than accepting a newer schema. If the rollback crosses a release that raised the schema version, new pods remain in `Init`. Such releases include patch releases: the Cassandra schema moved from `0.45` in v1.4.0 to `0.47` in v1.4.1. The Deployment stops progressing when it reaches its `maxUnavailable` limit, and the remaining pods on the newer image keep serving.

Before running `helm rollback`, compare the schema version in the database with the `version.go` of the target image for your driver, for example [`schema/cassandra/version.go`](https://github.com/cadence-workflow/cadence/blob/master/schema/cassandra/version.go). If they differ, roll the image back without the `wait-for-schema` init container, for example with `helm upgrade --post-renderer`. Restore the unmodified chart on the next upgrade.
